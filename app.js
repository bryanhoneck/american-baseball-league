let activeTab = "major";
let divisionViewMode = "region"; // "region" | "division"
...
function renderViewToggle() {
  return `
    <div class="view-toggle">
      <button class="view-toggle-btn${divisionViewMode === "region" ? " active" : ""}" data-view="region">Region View</button>
      <button class="view-toggle-btn${divisionViewMode === "division" ? " active" : ""}" data-view="division">Division View</button>
    </div>`;
}

function renderDivisionFlat(divKey, divData, expelledAbbr) {
  const allTeams = Object.entries(divData.regions)
    .flatMap(([region, teams]) => teams.map(t => ({ ...t, region })));
  const sorted = [...allTeams].sort((a, b) => (b.w / (b.w + b.l)) - (a.w / (a.w + a.l)));
  const leader = sorted[0];

  const rows = sorted.map((t, i) => {
    let rowClass = "";
    if (i === 0) rowClass = "rank-1";
    else if (expelledAbbr && t.abbr === expelledAbbr) rowClass = "rank-bottom";
    ...
    return `
      <tr class="${rowClass}">
        <td><span class="team-abbr">${t.abbr}</span><span class="team-full">${t.city} ${t.name}</span></td>
        <td class="region-col">${t.region}</td>
        <td>${t.w}</td>
        <td>${t.l}</td>
        <td class="pct">
          ${pct(t.w, t.l)}
  ...
  return `
    <div class="region-card division-flat-card">
      <table>
        <thead>
          <tr>
            <th>Team</th><th class="region-col">Region</th><th>W</th><th>L</th><th>PCT</th><th>GB</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
...
function renderDivision(divKey) {
  ...
  const body = divisionViewMode === "division"
    ? renderDivisionFlat(divKey, divData, expelledAbbr)
    : `<div class="regions-grid">${
        Object.entries(divData.regions).map(([name, teams]) => renderRegion(name, teams, expelledAbbr)).join("")
      }</div>`;

  return `
    ${renderLegend(divKey)}
    ${renderViewToggle()}
    <div class="division-block">${body}</div>`;
}
...
function renderReserve() {
  ...
  const sorted = [...allTeams].sort((a, b) => a.city.localeCompare(b.city));
  const rows = sorted.map(t => `
    <tr>
      <td>
        <span class="team-abbr">${t.abbr}</span>
        <span class="team-full">${t.name} (${t.city}, ${t.st})</span>
  ...
  return `
    <div class="division-block">
      <div class="region-card reserve-list-card">
...
function render() {
  ...
  app.querySelectorAll(".view-toggle-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      divisionViewMode = btn.dataset.view;
      render();
    });
  });
}
